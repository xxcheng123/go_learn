...
func (cc *CarCreate) SetCreateAt(t time.Time) *CarCreate {
...
func (cc *CarCreate) SetHostID(id int64) *CarCreate {
	cc.mutation.SetHostID(id)
	return cc
}

// SetNillableHostID sets the "host" edge to the User entity by ID if the given value is not nil.
func (cc *CarCreate) SetNillableHostID(id *int64) *CarCreate {
	if id != nil {
		cc = cc.SetHostID(*id)
	}
	return cc
...
func (cc *CarCreate) SetHost(u *User) *CarCreate {
	return cc.SetHostID(u.ID)
}
...
func (cc *CarCreate) check() error {
	...
	if _, ok := cc.mutation.CreateAt(); !ok {
		return &ValidationError{Name: "create_at", err: errors.New(`ent: missing required field "Car.create_at"`)}
	}
	return nil
}
...
func (cc *CarCreate) createSpec() (*Car, *sqlgraph.CreateSpec) {
	var (
		...
		_spec = sqlgraph.NewCreateSpec(car.Table, sqlgraph.NewFieldSpec(car.FieldID, field.TypeInt64))
	)
	...
	if nodes := cc.mutation.HostIDs(); len(nodes) > 0 {
